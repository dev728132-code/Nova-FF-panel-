  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      if (appliedPromo) {
        ...
        if (appliedPromo.fixed_discount) {
          finalAmount -= appliedPromo.fixed_discount;
        }
      ...
      if (appliedPromo) {
        try {
          ...
          if (currentPromo) {
            await supabase.from('promo_codes').update({ times_used: currentPromo.times_used + 1 }).eq('id', appliedPromo.id);
          }
        } catch(e) {}
      }
      ...
      try {
        await supabase.from('admin_notifications').insert({
          type: 'New Order',
          message: `New manual payment order submitted for ${product.name} by ${user.email}.`
        });
      } catch(e) {}
          ...
          <div className="lg:col-span-2">
            <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6 sticky top-24">
              ...
              <div className="space-y-4 mb-6 pb-6 border-b border-gray-800">
                ...
                {appliedPromo && (
                  <div className="flex justify-between text-sm">
                    <span className="text-green-500">Discount ({appliedPromo.code})</span>
                    <span className="text-green-500 font-medium font-mono">
                      -{appliedPromo.discount_percentage ? `${appliedPromo.discount_percentage}%` : `₹${appliedPromo.fixed_discount}`}
                    </span>
                  </div>
                )}
                <div className="flex justify-between text-sm">
